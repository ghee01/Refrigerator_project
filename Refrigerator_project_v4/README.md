...
| **공동 작업** | 테스트 · 문서화        | Swagger UI API 테스트, PostgreSQL·FastAPI·Streamlit 연동 테스트, 오류 수정 및 최종 기능 점검, GitHub 소스 관리, `README.md`·`TROUBLESHOOTING.md` 작성, 발표자료 및 최종 발표 준비                     |
...
* CSV 날짜 형식 변환 문제
* FastAPI 코드가 길어지면서 발생한 유지보수 문제
* Router와 Schema를 이용한 코드 모듈화
* Streamlit과 FastAPI 간 HTTP 통신 및 예외처리
* Streamlit 버튼의 중복 key 문제
* 데이터가 많아졌을 때 목록 표시를 위한 페이지네이션 처리
...
FastAPI, PostgreSQL, SQLAlchemy를 이용하여 실제 식재료 데이터를 관리하는 REST API를 구현하였습니다.
